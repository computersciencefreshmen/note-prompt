...
import { UpdatePromptData, Folder, Prompt } from '@/types'
...
export default function EditPromptPage() {
  ...
  const [formData, setFormData] = useState<UpdatePromptData>({
    title: '',
    content: '',
    folder_id: 0,
    tags: []
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        const [promptData, foldersData] = await Promise.all([
          api.prompts.getPrompt(promptId),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.prompts.updatePrompt(promptId, formData)
      router.push('/prompts')
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">修改你的提示词信息</p>
        </div>
